...
export default function TeacherMessagesPage() {
  ...
  const [messages, setMessages] = useState<TeacherMessageRow[]>([]);
  ...
  async function markMessagesAsRead(token: string, rows: TeacherMessageRow[]) {
  ...
  async function loadMessages({ markAsRead = true }: { markAsRead?: boolean } = {}) {
    try {
      ...
      if (!res.ok || !json?.ok) {
        setError(json?.error || "Erro ao carregar comunicados do professor.");
      ...
      const loadedMessages = (json.messages || []) as TeacherMessageRow[];
      ...
      if (markAsRead && loadedMessages.some((row) => !row.recipient?.readAt)) {
        ...
        const refreshed = await fetch("/api/teacher/messages", {
          headers: { Authorization: `Bearer ${token}` },
          cache: "no-store",
        });
        ...
        if (refreshed.ok && refreshedJson?.ok) {
          setMessages((refreshedJson.messages || []) as TeacherMessageRow[]);
          setSummary(
      ...
      <section className="overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-sm">
          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-200 md:text-base">
                Avisos oficiais enviados pela gestão escolar para professores e equipe.
                Ao abrir esta página, os comunicados recebidos são marcados como visualizados.
              </p>
        ...
        <div className="grid grid-cols-1 gap-4 p-4 md:grid-cols-4 md:p-6">
          <SummaryCard
            ...
            help="Total de comunicados disponíveis para este professor."
          />

          <SummaryCard
            label="Visualizados"
            value={String(summary.read || readCount)}
            help="Comunicados que já foram abertos no portal."
          />

          <SummaryCard
            label="Não lidos"
            value={String(summary.unread || unreadCount)}
            help="Após abrir esta tela, os novos comunicados são marcados como lidos."
          />
      ...
      <section className="overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 px-5 py-4 md:px-6">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Mensagens enviadas oficialmente pela direção, coordenação ou secretaria.
          </p>
        </div>
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex flex-wrap gap-2">
                          ...
                          <MessageTag variant="highlight">
                            {message.audienceLabel || "Professores"}
                          </MessageTag>

                          <MessageTag variant={isRead ? "success" : "default"}>
