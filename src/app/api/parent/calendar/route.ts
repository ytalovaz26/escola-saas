...
async function loadChildren(params: {
  ...
  const [studentsRes, activeClassesRes] = await Promise.all([
    supabaseAdmin
      .from("students")
  ...
  if (activeClassesRes.error) {
    throw new Error("Falha ao buscar turmas ativas: " + activeClassesRes.error.message);
  }

  const activeByStudent = new Map<string, any>();

  for (const row of activeClassesRes.data || []) {
    activeByStudent.set(String(row.student_id), row);
  }

  const classIds = Array.from(
    new Set(
      (activeClassesRes.data || [])
  ...
  const children: ParentChild[] = (studentsRes.data || [])
    .map((student: any) => {
      ...
      const cls = active?.class_id ? classById.get(String(active.class_id)) : null;
      ...
      return {
        ...
        activeClass: active?.class_id
          ? {
              classId: String(active.class_id),
...
async function loadOfficialScheduleEvents(params: {
  ...
  const events: ParentCalendarEvent[] = [];

  for (let i = 0; i < params.days; i++) {
    const date = addDaysISO(params.startDate, i);
    const weekday = getWeekday(date);

    const dayRows = rows.filter((row: any) => Number(row.weekday) === weekday);
    ...
    for (const row of dayRows) {
      ...
      for (const child of relatedChildren) {
        ...
        events.push({
          ...
          description:
            cleanText(row.notes) ||
            "Aula cadastrada na grade oficial da escola.",
          date,
...
async function loadSchoolEvents(params: {
  ...
  return (data || []).map((ev: any): ParentCalendarEvent => {
    return {
      id: `event-${ev.id}`,
      source: "school_event",
      type: "event",
  ...
  try {
    ...
    return jsonOk({
      ...
      meta: {
        startDate,
        days,
        source: "parent_calendar_with_official_schedule",
      },
    });
  } catch (e: any) {
