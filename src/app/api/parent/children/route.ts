...
export async function GET(req: Request) {
  try {
    ...
    if (uErr || !u?.user) {
      return jsonFail(401, "Invalid session");
    }
    ...
    if (!parent?.id) {
      return jsonFail(403, "Você não está cadastrado como responsável.");
    }
    ...
    for (const r of links || []) {
      relByStudent.set(String(r.student_id), (r as any).relationship ?? null);
    }
    ...
    if (classIds.length > 0) {
      ...
      classById = new Map((classes || []).map((c: any) => [String(c.id), c]));
    }

    const children = (students || [])
      .map((s: any) => {
        const active = activeByStudent.get(String(s.id)) || null;
        ...
        return {
          ...
          studentProfileUpdatedAt: s.student_profile_updated_at ?? null,
          relationship: relByStudent.get(String(s.id)) ?? null,
          active_class: active
            ? {
