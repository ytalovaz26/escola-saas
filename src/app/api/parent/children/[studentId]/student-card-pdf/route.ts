// src/app/api/parent/children/[studentId]/student-card-pdf/route.ts
...
async function requireParent(req: Request) {
  ...
  if (parentErr) {
    return {
      ok: false as const,
      res: jsonError("Erro ao validar responsável: " + parentErr.message, 500),
    };
  }
  ...
  if (!parent?.id || !parent?.school_id) {
    return {
      ok: false as const,
      res: jsonError("Você não está cadastrado como responsável.", 403),
    };
  }
...
export async function GET(
  ...
  try {
    ...
    const parentId = String(guard.parent.id);
    const schoolId = String(guard.parent.school_id);
    ...
    if (linkErr) {
      return jsonError("Erro ao validar vínculo com o aluno: " + linkErr.message, 500);
    }
    ...
    const schoolLogoBuffer = await getImageBuffer(
      school?.brand_logo_url || school?.logo_url || null
    );
