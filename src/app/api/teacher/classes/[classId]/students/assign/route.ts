// src/app/api/teacher/classes/[classId]/students/assign/route.ts
...
export async function POST(req: Request, ctx: { params: { classId: string } }) {
  try {
    ...
    if (!st?.id) return jsonError("Aluno não encontrado.", 404);
    if (st.school_id !== schoolId) return jsonError("Aluno não pertence à sua escola.", 403);
    ...
    if (!existing?.id) {
      ...
      return NextResponse.json({
        ok: true,
        classStudentId: inserted?.id ?? null,
        reactivated: false,
      });
    }

    // já existe -> garante ativo
