      ...
      const nextRows: GridRow[] = nextRoster.map((student) => {
        ...
        for (const subject of nextSubjects) {
          ...
          rowGrades[subject.id] =
            existing !== undefined && existing !== null && Number.isFinite(Number(existing))
              ? String(existing)
              : "";
        }
      ...
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        {!hasSubjects && !loading ? (
          <div className="p-6 text-sm text-slate-500">
            Esta turma ainda não possui disciplinas vinculadas. Peça para a escola cadastrar e vincular as disciplinas em <span className="font-medium">/school/subjects</span>.
          </div>
        ) : null}
                      ...
                      {subjects.map((subject) => (
                        <td key={subject.id} className="px-4 py-4">
                          <input
                            ...
                            onChange={(e) =>
                              updateGrade(row.student_id, subject.id, e.target.value)
                            }
                            placeholder="—"
