...
export async function GET(req: Request) {
  ...
  const [{ data: school, error: schoolErr }, { data: student, error: studentErr }] =
    await Promise.all([
      supabaseAdmin
        .from("schools")
  ...
  const className = classRow?.name || classId;
  const classGrade = classRow?.grade || "—";
  const classShift = classRow?.shift || "—";
  ...
  doc.text("Emissão:", pageW / 2, infoY);
  ...
  doc.text(brDateTimeFromIso(new Date().toISOString()), pageW / 2 + 50, infoY, {
    width: pageW / 2 - margin - 50,
  });
