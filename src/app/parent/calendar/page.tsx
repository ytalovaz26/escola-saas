...
export default function ParentCalendarPage() {
  ...
  async function loadCalendar(params?: {
    ...
    try {
      ...
      const res = await fetch(`/api/parent/calendar?${qs.toString()}`, {
        method: "GET",
        headers: {
      ...
      if (!res.ok || !json?.ok) {
        setError(json?.error || "Falha ao carregar rotina escolar.");
        return;
      }
      ...
      setError(e?.message || "Erro inesperado ao carregar rotina escolar.");
    } finally {
      setLoading(false);
    }
  }
  ...
  function changeStudent(studentId: string) {
    ...
    loadCalendar({ studentId, date: selectedDate });
  }

  function changeDate(date: string) {
    setSelectedDate(date);
    loadCalendar({ studentId: selectedStudentId, date });
  }
  ...
  useEffect(() => {
    ...
    loadCalendar({ studentId: urlStudentId, date: selectedDate });
        ...
        <section className="overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-sm">
            ...
            <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-200 md:text-base">
                  ...
                  salas, disciplinas e também eventos gerais da escola.
                </p>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => loadCalendar({ studentId: selectedStudentId, date: selectedDate })}
          ...
          <div className="grid grid-cols-1 gap-4 p-4 md:grid-cols-4 md:p-6">
            <MetricCard
              ...
              help="Aulas e eventos carregados."
              tone="blue"
            />
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm md:p-6">
            ...
            {nextEvent ? (
              ...
              <div className="mt-3 text-sm leading-6 text-slate-500">
                Nenhuma aula ou evento futuro identificado no momento.
              </div>
            )}
          </div>
        </section>
              ...
              <div>
                ...
                {dayEvents.length ? (
                  ...
                  <EmptyState
                    ...
                    description="Não existe aula ou evento cadastrado para esta data."
                  />
                )}
              </div>
            ) : view === "week" ? (
                    ...
                    <div key={date} className="rounded-[28px] bg-slate-50 p-4">
                      <div className="mb-3 flex items-center justify-between gap-3">
                        ...
                        <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-500">
                          {dateEvents.length} compromisso(s)
                        </span>
                      </div>
